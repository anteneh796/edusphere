    ...
    <div class="grid gap-4">
        @forelse ($absences as $absence)
            <x-card :title="$absence->student?->full_name ?? '—'" :subtitle="$absence->student?->classRoom?->gradeLevel?->name ?? ''">
                <div class="list-row">
                    <x-avatar :initials="$absence->guardian?->initials() ?? '–'" />
                    <div class="min-w-0 flex-1">
                        <div class="text-sm">
                            {{ $absence->guardian?->full_name ?? '—' }}
                ...
                @if (in_array($absence->status?->value ?? null, ['submitted', 'under_review'], true))
                    <details class="mt-3">
                        ...
                        <div class="flex flex-wrap items-end gap-3 mt-2">
                            <form method="POST" action="{{ route('guardian-services.absences.review', $absence) }}" class="flex flex-wrap items-end gap-3">
                                @csrf
