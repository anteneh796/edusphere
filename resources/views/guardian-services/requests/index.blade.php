    ...
    <div class="grid gap-4">
        @forelse ($requests as $request)
            <x-card :title="$request->reference_number" :subtitle="$request->subject">
                <div class="list-row">
                    <x-avatar :initials="$request->guardian?->initials() ?? '–'" />
                    <div class="min-w-0 flex-1">
                        <div class="text-sm">
                            {{ $request->guardian?->full_name ?? '—' }}
                            @if ($request->student)
                ...
                @if (in_array($request->status?->value ?? null, ['submitted', 'processing'], true))
                    <details class="mt-3">
                        ...
                        <div class="mt-2">
                            <form method="POST" action="{{ route('guardian-services.requests.process', $request) }}" class="flex flex-wrap items-end gap-3">
                                @csrf
                            ...
                            <form method="POST" action="{{ route('guardian-services.requests.process', $request) }}" class="flex flex-wrap items-end gap-3 mt-2">
                                @csrf
