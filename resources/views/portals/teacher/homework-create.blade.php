<x-layouts.app :title="__('New assignment')">
    <x-page-header :title="__('New assignment')" :description="__('Create a homework assignment for a class.')" />

    <x-card :title="__('Assignment details')">
        <form method="POST" action="{{ route('cms.teacher.homework.store') }}" class="grid gap-4 sm:grid-cols-2">
            @csrf

            <div class="sm:col-span-2">
                <x-select
                    name="class_subject_id"
                    :label="__('Class subject')"
                    :options="$subjects->mapWithKeys(fn ($s) => [$s->getKey() => ($s->subject?->name ?? '—') . ' — ' . ($s->classRoom?->name ?? '')])"
                    placeholder="{{ __('Select subject…') }}"
                    required />
            </div>

            <div class="sm:col-span-2">
                <x-input
                    name="title"
                    :label="__('Title')"
                    :value="old('title')"
                    placeholder="{{ __('e.g. Chapter 4 exercises') }}"
                    required />
            </div>

            <x-textarea name="instructions" :label="__('Instructions')" :value="old('instructions')" />

            <x-input
                name="due_on"
                type="date"
                :label="__('Due date')"
                :value="old('due_on')"
                required />

            <x-input
                name="assigned_on"
                type="date"
                :label="__('Assigned on')"
                :value="old('assigned_on', date('Y-m-d'))" />

            <x-input
                name="max_marks"
                type="number"
                :label="__('Max marks')"
                :value="old('max_marks')" />

            <x-select
                name="visibility"
                :label="__('Visibility')"
                :options="[
                    'class' => __('Class'),
                    'grade' => __('Grade'),
                    'school' => __('School'),
                ]"
                value="class" />

            <x-select
                name="status"
                :label="__('Status')"
                :options="[
                    'draft' => __('Draft'),
                    'published' => __('Published'),
                    'closed' => __('Closed'),
                ]"
                value="draft" />

            <div class="sm:col-span-2">
                <button type="submit" class="btn btn-primary">{{ __('Create assignment') }}</button>
                <a href="{{ route('cms.teacher.homework') }}" class="btn btn-ghost">{{ __('Cancel') }}</a>
            </div>
        </form>
    </x-card>
</x-layouts.app>