...
class GuardianServicesController extends Controller
{
    /* ------------------------------- Absence requests ----------------------------- */

    public function absencesIndex(): View
    {
        return view('guardian-services.absences.index', [
    ...
    public function absencesReview(AbsenceRequest $absence): RedirectResponse
    {
        ...
        $this->notifyGuardian($absence->guardian_id, $absence->student->full_name, [
            'type' => 'absence',
            'category' => 'attendance',
            'priority' => 'medium',
    ...
    private function notifyGuardian(string $guardianId, string $studentName, array $payload): void
    {
